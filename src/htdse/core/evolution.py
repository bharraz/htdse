...
def propagator(system, t, dim=None, t0: float = 0.0, **kwargs):
    """The propagator generated by `system` at one or more times.

    Dimension is inferred from `system.dim` or its subsystem registry. Pass
    `dim=` only for an external provider that supplies neither. Analytic
    unitary providers do not need a dimension.
    """
    if dim is None:
        dim = getattr(system, "dim", None)
        if dim is None:
            registered = getattr(system, "subsystems", None)
            if registered:
                dim = int(np.prod(tuple(registered.values())))
